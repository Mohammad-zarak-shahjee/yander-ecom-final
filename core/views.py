...
from .models import Item,Order,OrderItem,Address,Payment,Coupon,Refund, UserProfile,WishlistItem,Wishlist,Review
from django.utils import timezone
from django.shortcuts import redirect
from django.urls import reverse
from .forms import CheckoutForm,CouponForm,RefundForm,PaymentForm,ReviewForm,MyCustomSignupForm
...
class ItemDetailView(DetailView):
    model = Item
    #reviews = Review.objects.filter(status='True')  #id needed maybe
        ...
        if form.is_valid():
            ...
            if email_qs.exists():
                ...
                try:
                    ...
                    messages.info(self.request,"Please type in a valid Email!")
                    return redirect("core:request-refund")
                except ObjectDoesNotExist:
...
def addcomment(request,id):
    url = request.META.get('HTTP_REFERER') #gets last visited url

    if request.method =='POST': #check post
        form = ReviewForm(request.POST)
        if form.is_valid():
            data = Review() #creates relation with model
            ...
            data.save()  #saves the data in the table 
            messages.success(request,"Your review would be posted soon,Thanks for your interest.")
